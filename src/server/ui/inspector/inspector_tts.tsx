...
import { getVoices as getEdgeVoices } from "edge-tts";
...
const Windows: FC = () => {
  ...
  const handleUpdate = <K extends keyof TTS_State["windows"]>(
    key: K,
    v: TTS_State["windows"][K]
  ) => window.ApiServer.patchService("tts", (s) => (s.data.windows[key] = v));
  ...
  return (
    <>
      ...
      <InputSelect
        ...
        options={
          config?.devices.map((d) => ({ label: d.label, value: d.id })) || []
        }
...
const TikTok: FC = () => {
  ...
  const handleUpdate = <K extends keyof TTS_State["tiktok"]>(
    key: K,
    v: TTS_State["tiktok"][K]
  ) => window.ApiServer.patchService("tts", (s) => (s.data.tiktok[key] = v));
  return (
    <>
...
const Edge: FC = () => {
  const { t } = useTranslation();
  const data = useSnapshot(window.ApiServer.state.services.tts.data.edge);
  const state = useSnapshot(window.ApiServer.tts.serviceState);
  const handleUpdate = <K extends keyof TTS_State["edge"]>(
    key: K,
    v: TTS_State["edge"][K]
  ) => window.ApiServer.patchService("tts", (s) => (s.data.edge[key] = v));
  const { value: voices } = useSnapshot(edgeVoices);

  useEffect(() => {
    if (!voices.length) {
      getEdgeVoices().then((vs) => {
        edgeVoices.value = vs.map((v) => ({
  ...
  return (
    <>
      <Inspector.SubHeader>{t("tts.edge_title")}</Inspector.SubHeader>
      <Inspector.Deactivatable
        active={state.status === ServiceNetworkState.disconnected}
      >
        <InputNativeAudioOutput
          label="common.field_output_device"
          value={data.device}
...
const Native: FC = () => {
  ...
  const handleUpdate = <K extends keyof TTS_State["native"]>(
    key: K,
    v: TTS_State["native"][K]
  ) => window.ApiServer.patchService("tts", (s) => (s.data.native[key] = v));
  const state = useSnapshot(window.ApiServer.tts.serviceState);
  ...
  useEffect(() => {
    ...
    if (voices.length) {
      ...
      window.speechSynthesis.onvoiceschanged = () => {
        let options = window.speechSynthesis
          .getVoices()
          .map((v) => ({ label: v.name, value: v.voiceURI }));
        setVoices(options);
      };
    }
  }, []);
...
const Azure: FC = () => {
  ...
  const handleUpdate = <K extends keyof TTS_State["azure"]>(
    key: K,
    v: TTS_State["azure"][K]
  ) => (window.ApiServer.state.services.tts.data.azure[key] = v);
  const state = useSnapshot(window.ApiServer.tts.serviceState);
  ...
  useEffect(() => {
    ...
    if (!data.language || !data.voice) return;

    const voice = azureVoices[data.language].find(
      (voice) => voice[0] === data.voice
    );
    ...
    <>
      ...
      {voiceStyles.length > 0 && (
        <InputSelect
          ...
          options={voiceStyles}
          label="tts.azure_voice_style"
          onValueChange={(e) => handleUpdate("voiceStyle", e)}
        />
      )}
      {voiceRoles.length > 0 && (
        <InputSelect
          value={data.voiceRole}
          options={voiceRoles}
          label="tts.azure_voice_role"
          onValueChange={(e) => handleUpdate("voiceRole", e)}
        />
      )}
      <InputSelect
        options={[
        ...
        label="tts.azure_voice_volume"
        value={data.voiceVolume}
        onValueChange={(e) => handleUpdate("voiceVolume", e)}
      />
      <InputSelect
        options={[
        ...
        label="tts.azure_voice_rate"
        value={data.voiceRate}
        onValueChange={(e) => handleUpdate("voiceRate", e)}
      />
      <InputSelect
        options={[
        ...
        label="tts.azure_voice_pitch"
        value={data.voicePitch}
        onValueChange={(e) => handleUpdate("voicePitch", e)}
      />
      <InputSelect
        options={[
          { label: "x-low", value: "x-low" },
        ...
        label="tts.azure_voice_range"
        value={data.voiceRange}
        onValueChange={(e) => handleUpdate("voiceRange", e)}
      />
      <InputRange
        value={data.volume}
        onChange={(e) => handleUpdate("volume", e.target.value)}
...
const VoiceVox: FC = () => {
  const handleQuery = async () => {
    let resp = await fetch(
      "http://127.0.0.1:50021/audio_query?text=text&speaker=1",
      {
        method: "POST",
        body: JSON.stringify({
          query: {
...
const edgeVoices = proxy<{ value: InputSelectOption[] }>({
  value: [],
});
...
const UberDuck: FC = () => {
  ...
  const state = useSnapshot(window.ApiServer.tts.serviceState);
  const handleUpdate = <K extends keyof TTS_State["uberduck"]>(
    key: K,
    v: TTS_State["uberduck"][K]
  ) => window.ApiServer.patchService("tts", (s) => (s.data.uberduck[key] = v));
  const { value: voices } = useSnapshot(uberduckVoices);
  const [loadingVoices, setLoadingVoices] = useState(false);

  const loadVoices = () => {
    if (!data.api_key || !data.secret_key) return;
    ...
    invoke("plugin:uberduck_tts|get_voices", {
      ...
      .then((res) => {
        if (Array.isArray(res))
          uberduckVoices.value = res.map((v) => ({
            value: v.voicemodel_uuid,
  ...
  return (
    <>
      ...
      >
        ...
        />
        <InputText
          label="tts.uberduck_api_key"
          value={data.api_key}
          onChange={(e) => handleUpdate("api_key", e.target.value)}
        />
        <InputText
          label="tts.uberduck_secret"
          type="password"
          value={data.secret_key}
          onChange={(e) => handleUpdate("secret_key", e.target.value)}
        />
        <Inspector.Description>
          <span className="text-base-content/60 text-xs mt-2">
            {t("tts.uberduck_voice_desc")}
            <br />
            <a
              className="link link-primary link-hover"
              target="_blank"
              href="https://app.uberduck.ai/speak#mode=tts-reference&voice=zwf"
            >
              {t("tts.uberduck_voice_desc_link")}
            </a>
          </span>
        </Inspector.Description>
        <InputSelect
        ...
        <button
          onClick={loadVoices}
          className={classNames("btn btn-xs disabled", {
            loading: loadingVoices,
            "btn-disabled": !data.api_key || !data.secret_key,
          })}
        >
          {loadingVoices
            ? t("tts.uberduck_voice_status_updating")
            : t("tts.uberduck_voice_status_reload")}
        </button>
        <InputRange
          label="common.field_volume"
          value={data.volume}
...
const WordsReplacementModal: FC = () => {
  ...
  const up = <K extends keyof TTS_State>(key: K, v: TTS_State[K]) =>
    window.ApiServer.patchService("tts", (s) => (s.data[key] = v));
  ...
  return (
    <Modal.Body width={420}>
      ...
      <Modal.Content>
        <div className="p-4">
          <InputCheckbox
            label="word_replacements.field_ignore_case"
            value={data.data.replaceWordsIgnoreCase}
            onChange={(v) => up("replaceWordsIgnoreCase", v)}
          />
          <InputMapObject
            keyPlaceholder={t("word_replacements.label_dictionary_key")}
            valuePlaceholder={t("word_replacements.label_dictionary_value")}
            addLabel="Add word"
            value={{ ...data.data.replaceWords }}
            onChange={(e) => up("replaceWords", e)}
            label=""
          />
        </div>
      </Modal.Content>
...
NiceModal.register("tts-replacements", (props) => (
  <Modal.Base {...props}>
    <WordsReplacementModal />
  </Modal.Base>
));
...
const Inspector_TTS: FC = () => {
  ...
  const up = <K extends keyof TTS_State>(key: K, v: TTS_State[K]) =>
    window.ApiServer.patchService("tts", (s) => (s.data[key] = v));
  const handleStart = (v: boolean) =>
    (window.ApiServer.state.services.tts.showActionButton = v);
  const state = useSnapshot(window.ApiServer.tts.serviceState);
  ...
  return (
    <Inspector.Body>
      <Inspector.Header>
        <RiChatVoiceFill /> {t("tts.title")}
      </Inspector.Header>
      <Inspector.Content>
        <InputCheckbox
          label="common.field_action_bar"
          value={data.showActionButton}
          onChange={handleStart}
        />
        <InputCheckbox
          label="common.field_auto_start"
          value={data.data.autoStart}
          onChange={(e) => up("autoStart", e)}
        />
        <InputCheckbox
          label="tts.field_stop_with_stream"
          value={data.data.stopWithStream}
          onChange={(e) => up("stopWithStream", e)}
        />
        <span
          className="link link-accent link-hover font-semibold flex items-center gap-2 text-sm"
          onClick={handleShowReplacements}
        >
          <RiCharacterRecognitionFill /> {t("common.btn_edit_replacements")}
        </span>
        <Inspector.Deactivatable
          active={state.status === ServiceNetworkState.disconnected}
        >
          <InputTextSource
            ...
            onChange={(e) => up("source", e)}
          />
          <InputCheckbox
            label="common.field_use_keyboard_input"
            value={data.data.inputField}
            onChange={(e) => up("inputField", e)}
          />
          <InputSelect
            label="common.field_service"
            value={data.data.backend}
            options={[
              ...
              { label: "Edge", value: TTS_Backends.edge },
        ...
        {data.data.backend === TTS_Backends.edge && <Edge />}
        {/* {data.data.backend === TTS_Backends.voicevox && <VoiceVox />} */}
        <ServiceButton
          status={state.status}
          onStart={() => window.ApiServer.tts.start()}
          onStop={() => window.ApiServer.tts.stop()}
        />
      </Inspector.Content>
    </Inspector.Body>
  );
